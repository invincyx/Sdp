      ...
      if (rows.length > 0) {
        for (const row of rows) {
          ...
          try {
            ...
            const hitSdp = await hitSDP({token: token.access_token, request: featureId, requestId: Math.floor(row.trans_id), msisdn: Math.floor(row.receiver), planId: row.P_Code })
            console.log(hitSdp);
